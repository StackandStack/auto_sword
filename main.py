...
if __name__ == '__main__':
    ...
    array = [int(re.findall(r'[0-9]+', file)[0]) for file in os.listdir("./") if file.endswith(".npy")]
    if len(array) == 0:
        number = 1
    else:
        number = np.max(array) + 1
    while True:
        pag.PAUSE = 0.05
        with mss.mss() as sct:
            ...
            print(left_image_nm + " : " + str(left_mean))
            print(right_image_nm + " : " + str(right_mean))
            if (left_image_nm == 'SWORD') and (right_image_nm == 'BOMB' or right_image_nm == 'POISON'):
                print('TAP left')
                test_set.append({'number': iter, 'dataset': {'y': [1, 0], 'x': [left_img, right_img]}, 'suceess': 1})
                click(x=left_button[0], y=left_button[1])
                breakp = 0
            elif (right_image_nm == 'SWORD') and (left_image_nm == 'BOMB' or left_image_nm == 'POISON'):
                print('TAP right')
                test_set.append({'number': iter, 'dataset': {'y': [0, 1], 'x': [left_img.flatten(), right_img]}, 'suceess': 1})
                click(x=right_button[0], y=right_button[1])
                breakp = 0
            elif left_image_nm == 'JEWEL' and right_image_nm == 'JEWEL':
                print('FEVER')
                test_set.append({'number': iter, 'dataset': {'y': [1, 1], 'x': [left_img, right_img]}, 'suceess': 1})
                click(x=left_button[0], y=left_button[1])
                click(x=right_button[0], y=right_button[1])
                breakp = 0
            else:
                test_set.append({'number': iter, 'dataset': {'y': [0, 0], 'x': [left_img, right_img]}, 'suceess': 1})
                breakp += 1
                if breakp >= 100:
                    np.save(f'testset_{number}.npy', test_set)
                    break
